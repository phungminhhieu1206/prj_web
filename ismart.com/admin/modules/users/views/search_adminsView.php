...
#Số bản ghi/trang
$num_per_page = 3;
# search post and get list admins
if(!empty($_GET['value'])){
    $value = $_GET['value'];
}
...
$total_approved = db_num_rows("SELECT* FROM  `tbl_admins` Where `admin_status`= 'Approved' AND CONVERT(`username` USING utf8) LIKE '%$value%' OR CONVERT(`fullname` USING utf8) LIKE '%$value%'");
# Bài viết chờ xét duyệt
$total_waitting = db_num_rows("SELECT* FROM  `tbl_admins` Where `admin_status`= 'Waitting...' AND CONVERT(`username` USING utf8) LIKE '%$value%' OR CONVERT(`fullname` USING utf8) LIKE '%$value%'");
# Bài viết đã bị xóa
$total_trash = db_num_rows("SELECT* FROM  `tbl_admins` Where `admin_status`= 'Trash' AND CONVERT(`username` USING utf8) LIKE '%$value%' OR CONVERT(`fullname` USING utf8) LIKE '%$value%'");
?>
        ...
        <div id="content" class="fl-right">
            <div class="section" id="detail-page">
                <div class="section-detail">
                    ...
                    <form method="POST" action="?mod=users&controller=team&action=apply_admins&page_id=<?php echo $page;?>&value=<?php if(!empty($value)){ echo $value;}?>">
                        <div class="actions">
